...
const standardQuestions = (element, selectorCit = 'standart', key, openAll = false, arlistLength) => {
  ...
  div.innerHTML = `
      ...
      <div class="wrapper_wrap">
          <div class="row" style="display: flex; flex-direction: column; gap: 13px;">
             ...
             ${global.mobaleMOde && !openAll && key !== arlistLength ? ` 
             <div>
  ...
  window.openall = false; // console.log(`${key} !== ${arlistLength} ${key !== arlistLength}`);
...
const startaStage = () => {
  ...
  btnsStart.forEach(item => {
    item.addEventListener('click', e => {
      ...
      Object(_services_changeLevel__WEBPACK_IMPORTED_MODULE_0__["default"])(e.target); // changeLevel(e.target, selectorListQ, btnsStart);
    });
  });
};
...
const whereStay = (selector = false) => {
  try {
    // if (selector) {
    // 	return selector.addEventListener('click', e => {
    // 		console.log('hello===== is hereStay ');
    // 		whereSteyFunc(e, p);
    // 	});
    // }
    document.querySelector('#wheryIsty').addEventListener('click', e => Object(_services_whereSteyFunc__WEBPACK_IMPORTED_MODULE_0__["default"])(e, p));
  } catch (error) {
    console.error(error);
  }
};
...
// import { CountElementIsDOM, mobailCle } from './whereStCicle';
...
} // function addLisenCicle(selectorListQ) {
// 	mobailCle(CountElementIsDOM(), selectorListQ - CountElementIsDOM(), true);
// }
...
const wrapperTutle = document.querySelector('.wrapperTutle'),
      ...
      body = document.body,
      html = document.documentElement;
let selectorEndIteration = 0;

const changeLevel = targetBtn => {
  // console.log(`вызво changeLevel ${targetBtn} ${selectorListQ} ${btnsStart}`);
  ...
  let time,
      ...
      btnsStart = document.querySelectorAll('.button'); // console.log(selectorEndIteration);
  ...
  switch (targetBtn.classList[2]) {
    case 'tick':
      selectorEndIteration = window.objectAllCor[localSel];
      sel = 'tick';
      break;
    ...
    case 'flower':
      ...
      sel = 'flower';
      break;
  } // console.log(selectorEndIteration);
  ...
  if (global.mobaleMOde && width <= 775) {
    ...
    elementList.removeEventListener('click', _LitlModules__WEBPACK_IMPORTED_MODULE_1__["scrollDown"]); // elementList.removeEventListener('click', addLisenCicle);

    elementList.addEventListener('click', () => {
      console.log(selectorEndIteration);
      Object(_whereStCicle__WEBPACK_IMPORTED_MODULE_5__["mobailCle"])(Object(_whereStCicle__WEBPACK_IMPORTED_MODULE_5__["CountElementIsDOM"])(), selectorEndIteration - Object(_whereStCicle__WEBPACK_IMPORTED_MODULE_5__["CountElementIsDOM"])(), true);
      selectorEndIteration = 0;
      console.log(selectorEndIteration);
    }, {
      once: true
    }); // console.log(selectorEndIteration);
    ...
    // 	mobailCle(
    // 		CountElementIsDOM(),
    // 		selectorEndIteration - CountElementIsDOM(),
    // 		true
    // 	);
    // 	console.log(selectorEndIteration);
    // 	selectorEndIteration = 0;
    // 	console.log(selectorEndIteration);
    // };
    // elementList.addEventListener('click', addLisenCicle, { once: true });
...
const mobailCle = (startIndex, numberQuestion, allDowen = false) => {
  ...
  const modeBtnOPenAll = () => {
    ...
    elementList.addEventListener('click', _LitlModules__WEBPACK_IMPORTED_MODULE_2__["scrollDown"]); // elementList.removeEventListener('click', addLisenCicle);
  };

  if (startIndex > numberQuestion + startIndex) {
...
const CountElementIsDOM = () => {
  return document.getElementsByClassName('tinRightIn').length || 0;
};
...
const whereSteyFunc = (e, p) => {
  ...
  if (selectorQuestionopen !== selectorQuestionLevel || !selectorQuestionopen) {
    ...
    Object(_changeLevel__WEBPACK_IMPORTED_MODULE_3__["default"])(document.querySelector('.' + selectorQuestionLevel));
